...
import { Calendar, Users, ShoppingCart, Loader2, UtensilsCrossed, ExternalLink, Clock } from 'lucide-react';
...
export default function MealPlanner() {
  ...
  const sendEmailReminders = async (planData) => {
    try {
      // Create a formatted email with the meal plan
      const emailContent = formatMealPlanForEmail(planData);
      ...
      const response = await fetch('https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash-exp:generateContent?key=' + 'YOUR_GEMINI_API_KEY', {
        ...
        body: JSON.stringify({
          contents: [{
            parts: [{
              text: `Generate a brief confirmation message that weekly meal plan reminders will be sent to ${formData.email} every ${formData.generateDay}. Keep it friendly and concise (2-3 sentences).`
            }]
          }],
          generationConfig: {
            temperature: 0.7,
